...
extern "C" {
  void FfiBridge(word* regular, int regularSize, word* stack, int stackSize,
    word* sse, int sseSize, word foreign, word sseFlags);
}

typedef word (*P_BRIDGE)(word*, int, word*, int, word*, int, word, word);
typedef int32 (*I32_BRIDGE)(word*, int, word*, int, word*, int, word, word);
typedef int64 (*I64_BRIDGE)(word*, int, word*, int, word*, int, word, word);
typedef float (*F32_BRIDGE)(word*, int, word*, int, word*, int, word, word);
typedef double (*F64_BRIDGE)(word*, int, word*, int, word*, int, word, word);
typedef void (*V_BRIDGE)(word*, int, word*, int, word*, int, word, word);

class FFIFrameBuilder {
 public:
  FFIFrameBuilder():floatingPointArgsFlags(0) {}

  void WordArgument(word v) {
    if (registerArgs.size() < kLastCoreRegister) {
      registerArgs.PushBack(v);
    } else {
      stackArgs.PushBack(v);
    }
  ...
  void Float32Argument(float v) {
    word w = *reinterpret_cast<uint32*>(&v);
  ...
  void Float64Argument(double v) {
    word w = *reinterpret_cast<word*>(&v);
    if (floatingPointArgs.size() < kLastFloatingPointRegister) {
      floatingPointArgsFlags |= 1 << floatingPointArgs.size();
      floatingPointArgs.PushBack(w);
    } else {
      stackArgs.PushBack(w);
    }
 ...
 private:
  const size_t kLastCoreRegister = 6;
  const size_t kLastFloatingPointRegister = 8;
  Vector<word> registerArgs, stackArgs;
  Vector<word> floatingPointArgs;
  word floatingPointArgsFlags;
};
