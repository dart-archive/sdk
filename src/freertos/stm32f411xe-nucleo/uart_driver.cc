...
#include "src/freertos/stm32f411xe-nucleo/uart_driver.h"
...
void UartDriverImpl::InterruptHandler() {
  ...
  if ((__HAL_UART_GET_FLAG(uart_, UART_FLAG_PE) != RESET) &&
  ...
  if ((__HAL_UART_GET_FLAG(uart_, UART_FLAG_FE) != RESET) &&
  ...
  if ((__HAL_UART_GET_FLAG(uart_, UART_FLAG_NE) != RESET) &&
      (__HAL_UART_GET_IT_SOURCE(uart_, UART_IT_ERR) != RESET)) {
    // Noise error
  ...
  if ((__HAL_UART_GET_FLAG(uart_, UART_FLAG_RXNE) != RESET) &&
      (__HAL_UART_GET_IT_SOURCE(uart_, UART_IT_RXNE) != RESET)) {
    // Incoming character
    uint8_t byte = (uint8_t)(uart_->Instance->DR & 0xff);
  ...
  if ((__HAL_UART_GET_FLAG(uart_, UART_FLAG_TC) != RESET) &&
