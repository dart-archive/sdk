...
void PerformanceService::countTreeNodesAsync(TreeNodeBuilder node, void (*callback)(int32_t)) {
  node.InvokeMethodAsync(service_id_, kCountTreeNodesId_, Unwrap_int32_8, reinterpret_cast<void*>(callback));
}
