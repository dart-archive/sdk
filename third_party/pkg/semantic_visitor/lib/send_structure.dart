...
class LazyAndStructure<R, A> implements SendStructure<R, A> {
  const LazyAndStructure();

  R dispatch(SemanticSendVisitor<R, A> visitor, Send node, A arg) {
    return visitor.visitLazyAnd(
        node,
        node.receiver,
        node.arguments.single,
        arg);
...
class LazyOrStructure<R, A> implements SendStructure<R, A> {
  const LazyOrStructure();

  R dispatch(SemanticSendVisitor<R, A> visitor, Send node, A arg) {
    return visitor.visitLazyOr(
        node,
        node.receiver,
        node.arguments.single,
        arg);
